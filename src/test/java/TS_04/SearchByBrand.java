...
public class SearchByBrand extends Utility.BeginnerClass {
	...
	public void startBrowser() throws IOException {
		...
		if (ExtentReport.log == null)
			ExtentReport.log = ExtentReport.reports.createTest("Search new bikes by Brand");
	...
	public void setBrandFilter() {
		...
		// Logging the execution process in HTML
		report.display(logger, "Selecting filter for brand in the search page");

		// Applying Filter
		newBikesSearch.selectBrand("brand");

		// Updating the status of execution to RTM
		xL.updateResultToRTM("<New Bikes Search> page is displayed with applied filter");

		// Logging the execution process in HTML
		report.select(logger, "Brand from filter is selected");
	...
	public void outputDisplay() {
		...
		List<String> Bikes = newBikesSearch.getBrand();
		...
		System.out.println("New  Bikes (Brand Filter ) :");
		...
		// Logging the execution process in HTML
		report.display(logger, "New Honda Bikes are asserted and displayed");
