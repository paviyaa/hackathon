...
public class SearchByMileage extends Utility.BeginnerClass {
	...
	public void startBrowser() throws IOException {
		...
		if (ExtentReport.log == null)
			ExtentReport.log = ExtentReport.reports.createTest("Search new bikes by Mileage");
	...
	public void setEngineFilter() {
		...
		report.display(logger, "Selecting filter for mileage in the search page");

		// Applying Filter
		newBikesSearch.selectMileage();

		// Logging the execution process in HTML
		report.select(logger, "Mileage from filter is selected");
	...
	public void outputDisplay() {
		...
		List<String> Bikes = newBikesSearch.getMileage();
		...
		for (String bike : Bikes) {
			Assert.assertTrue(Integer.valueOf(bike.replace("kmpl", "").trim()) <= 50
					&& Integer.valueOf(bike.replace("kmpl", "").trim()) >= 30);
		...
		// Logging the execution process in HTML
		report.display(logger, "New Bikes with Selected Mileage are asserted and displayed");
	}
